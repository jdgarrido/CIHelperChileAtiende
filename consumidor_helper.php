...
function APIAllFichas($aParams = array()) {
    ...
    if ($aParams['query'])
        $url_sufix = $url . '&query=' . str_replace(' ', '%20', $aParams['query']);
    if (intval($aParams['maxResults']))
        $url_sufix = $url . '&maxResults=' . $aParams['maxResults'];
    if ($aParams['nextPageToken'])
        $url_sufix = $url . '&pageToken=' . $aParams['nextPageToken'];
